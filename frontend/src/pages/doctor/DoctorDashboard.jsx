...
const DoctorDashboard = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const doctorRes = await fetch(`http://localhost:8081/api/doctors/user/${user.id}`);
      if (!doctorRes.ok) {
      ...
      const appRes = await fetch(`http://localhost:8081/api/appointments/doctor/${doctorData.id}`);
  ...
  const todaysAppointments = appointments.filter(app => app.date === today || app.status === 'Sắp tới' || app.status === 'Chờ xác nhận');

  if (loading) return <div className="p-6 text-center">Đang tải lịch hẹn...</div>;
          ...
          <table className="w-full whitespace-nowrap">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Mã BN</th>
              ...
              {todaysAppointments.length > 0 ? (
                todaysAppointments.map((app) => (
                  <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-900">{app.patient?.fullName || 'N/A'}</div>
                      <div className="text-sm text-gray-500">{app.patient?.phone || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                        app.status === 'Sắp tới' ? 'bg-blue-100 text-blue-800' :
                        app.status === 'Đã khám' ? 'bg-green-100 text-green-800' :
