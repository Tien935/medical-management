...
const ManagePatients = () => {
  ...
  const handlePatientSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchPatients();
        alert(editingPatient ? "Cập nhật thành công!" : "Thêm bệnh nhân thành công!");
      } else {
  ...
  const handleViewDetails = async (patient) => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/api/appointments/patient/${patient.id}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const upcomingAppointments = appointments.filter(a => a.status === 'Sắp tới' || a.status === 'Chờ xác nhận');
  const historyAppointments = appointments.filter(a => a.status === 'Đã khám' || a.status === 'Đã hủy');
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800">Quản lý Bệnh nhân</h2>
          <p className="text-slate-500 font-semibold mt-1">Danh sách bệnh nhân đã đăng ký trên hệ thống</p>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  <th className="px-6 py-4 font-bold text-slate-400 uppercase text-xs tracking-wider">Mã BN</th>
                  <th className="px-6 py-4 font-bold text-slate-400 uppercase text-xs tracking-wider">Thông tin bệnh nhân</th>
                    ...
                    <tr key={patient.id} className="hover:bg-slate-50 transition group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-slate-800 text-base">{patient.fullName || 'Chưa cập nhật'}</p>
                            <p className="text-xs text-slate-400 font-medium">Username: {patient.username}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center text-sm text-slate-600">
                            ...
                            <span className={patient.email ? "" : "italic text-slate-400"}>{patient.email || 'Chưa cập nhật'}</span>
                          </div>
                          <div className="flex items-center text-sm text-slate-600">
                            <i className="fas fa-phone-alt w-5 text-slate-400"></i>
                            <span className={patient.phone ? "" : "italic text-slate-400"}>{patient.phone || 'Chưa cập nhật'}</span>
                          </div>
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl my-8 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-start shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-slate-800">{selectedPatient.fullName}</h3>
                  <div className="flex items-center gap-4 mt-2 text-sm text-slate-500 font-semibold">
                    <span className="flex items-center gap-1"><i className="fas fa-id-card text-slate-400"></i> {formatPatientId(selectedPatient.id)}</span>
                    <span className="flex items-center gap-1"><i className="far fa-envelope text-slate-400"></i> {selectedPatient.email || 'N/A'}</span>
                    <span className="flex items-center gap-1"><i className="fas fa-phone-alt text-slate-400"></i> {selectedPatient.phone || 'N/A'}</span>
                  </div>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} className="w-10 h-10 rounded-full bg-slate-50 text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition flex items-center justify-center">
            ...
            <div className="p-6 flex-1 overflow-y-auto bg-slate-50/50">
              ...
              <div className="flex gap-4 border-b border-slate-200 mb-6">
                ...
                >
                  ...
                  <span className="ml-2 bg-slate-100 text-slate-600 py-0.5 px-2 rounded-full text-xs">{upcomingAppointments.length}</span>
                </button>
                <button 
                ...
                >
                  ...
                  <span className="ml-2 bg-slate-100 text-slate-600 py-0.5 px-2 rounded-full text-xs">{historyAppointments.length}</span>
                </button>
              </div>
              ...
              {loadingAppointments ? (
                <div className="py-12 text-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-teal-600 mx-auto"></div>
                  <p className="text-slate-400 mt-4 font-medium text-sm">Đang tải dữ liệu...</p>
                </div>
              ) : (
                  ...
                  {activeTab === 'upcoming' && (
                    upcomingAppointments.length > 0 ? (
                          ...
                          <div key={app.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div className="flex gap-4 items-start">
                              <div className="bg-teal-50 w-14 h-14 rounded-xl flex flex-col items-center justify-center text-teal-600 shrink-0 border border-teal-100">
                                <span className="text-xs font-bold uppercase">{app.date ? new Date(app.date).toLocaleDateString('vi-VN', { month: 'short' }) : ''}</span>
                                <span className="text-xl font-black leading-none">{app.date ? new Date(app.date).getDate() : ''}</span>
                              </div>
                              <div>
                                <h4 className="font-bold text-slate-800 flex items-center gap-2">
                                  {app.id}
                                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${app.status === 'Sắp tới' ? 'bg-blue-100 text-blue-600' : 'bg-amber-100 text-amber-600'}`}>
                                    {app.status}
                                  </span>
                                </h4>
                                <p className="text-sm font-semibold text-slate-500 mt-1"><i className="far fa-clock mr-1"></i> {app.time}</p>
                                {app.doctor && <p className="text-sm font-semibold text-slate-600 mt-1"><i className="fas fa-user-md mr-1 text-slate-400"></i> Bác sĩ: {app.doctor.name}</p>}
                              </div>
                            </div>
                            {app.notes && (
                              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-sm text-slate-600 md:max-w-xs">
                                <span className="font-bold text-slate-700">Lý do khám:</span> {app.notes}
                              </div>
                            )}
                          </div>
                        ))}
                      ...
                      <div className="text-center py-12 bg-white rounded-2xl border border-slate-200 border-dashed">
                        ...
                        <p className="text-slate-500 font-medium">Bệnh nhân không có lịch hẹn nào sắp tới.</p>
                      </div>
                    )
                  )}
                  ...
                  {activeTab === 'history' && (
                    historyAppointments.length > 0 ? (
                          ...
                          <div key={app.id} className="relative pl-8">
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                              <div className="flex justify-between items-start mb-3">
                                <div>
                                  <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-md mb-2 inline-block">{app.date} • {app.time}</span>
                                  <h4 className="font-black text-slate-800 text-lg">Khám {app.doctor?.specialty?.name || 'Tổng quát'}</h4>
                                </div>
                                <span className={`px-3 py-1 rounded-full text-xs font-black uppercase ${app.status === 'Đã khám' ? 'bg-teal-100 text-teal-600' : 'bg-red-100 text-red-600'}`}>
                                  {app.status}
                                </span>
                              </div>
                              <div className="text-sm font-semibold text-slate-600 mb-4">
                                <i className="fas fa-user-md text-slate-400 mr-2"></i> Bác sĩ phụ trách: <span className="text-slate-800">{app.doctor?.name}</span>
                              </div>
                              {app.status === 'Đã khám' && app.notes && (
                                <div className="bg-teal-50/50 p-4 rounded-xl border border-teal-100/50">
                                  <h5 className="text-xs font-black text-teal-800 uppercase tracking-wider mb-2 flex items-center gap-2">
                                    <i className="fas fa-notes-medical"></i> Kết luận / Ghi chú
                                  </h5>
                      ...
                      <div className="text-center py-12 bg-white rounded-2xl border border-slate-200 border-dashed">
                        ...
                        <p className="text-slate-500 font-medium">Chưa có lịch sử khám bệnh.</p>
                      </div>
                    )
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setShowPatientForm(false)} className="text-slate-400 hover:text-slate-600 w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-200 transition">
                <i className="fas fa-times text-xl"></i>
              </button>
            </div>
            ...
            <form onSubmit={handlePatientSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="email" className="w-full p-3 rounded-xl border border-slate-200 focus:border-teal-500 outline-none placeholder-slate-300" placeholder="email@example.com" value={patientFormData.email} onChange={e => setPatientFormData({...patientFormData, email: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-600 mb-1">Số điện thoại</label>
                  <input type="text" className="w-full p-3 rounded-xl border border-slate-200 focus:border-teal-500 outline-none placeholder-slate-300" placeholder="0912345678" value={patientFormData.phone} onChange={e => setPatientFormData({...patientFormData, phone: e.target.value})} />
                </div>
              </div>

              <div className="pt-6 flex justify-end gap-3">
                <button type="button" onClick={() => setShowPatientForm(false)} className="px-6 py-2.5 rounded-full font-bold text-slate-600 hover:bg-slate-100 transition">
                  Hủy
                </button>
                <button type="submit" className="px-6 py-2.5 rounded-full font-bold text-white bg-teal-600 hover:bg-teal-700 shadow-lg shadow-teal-600/30 transition">
