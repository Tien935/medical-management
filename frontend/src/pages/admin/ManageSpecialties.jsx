      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-1">
            Danh sách các chuyên khoa tại hệ thống
          </p>
        </div>

        <div className="flex flex-nowrap items-center gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
          <div className="relative w-full md:w-64 min-w-[180px]">
            <input
              type="text"
              placeholder="Tìm tên, mô tả..."
              className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:border-teal-500 transition-colors text-sm"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <i className="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
          </div>
          <div className="bg-teal-50 text-teal-600 px-4 py-2 rounded-xl font-bold text-xs whitespace-nowrap border border-teal-100">
            Tổng: {filteredSpecialties.length}
          </div>
          <button
            onClick={handleAddNew}
            className="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded-xl shadow-lg transition flex items-center whitespace-nowrap shrink-0 text-sm"
          >
          ...
          filteredSpecialties.map((specialty) => (
            ...
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="w-10 h-10 rounded-xl bg-white/90 backdrop-blur shadow-lg text-teal-600 flex items-center justify-center hover:bg-teal-600 hover:text-white transition"
                  >
                    <i className="fas fa-edit"></i>
                  </button>
                  <button
                    onClick={() => handleDelete(specialty.id)}
                    className="w-10 h-10 rounded-xl bg-white/90 backdrop-blur shadow-lg text-red-600 flex items-center justify-center hover:bg-red-600 hover:text-white transition"
                  >
              ...
              <div className="p-6">
                ...
                <p className="text-slate-500 text-sm font-semibold line-clamp-3 mb-4">
